...
bool HybridAStar::CheckCollision(const double& x, const double& y, const double& theta) {
    ...
    if (!LineCheck(pt1.x(), pt1.y(), pt0.x(), pt0.y())) return false;
    if (!LineCheck(pt2.x(), pt2.y(), pt1.x(), pt1.y())) return false;
    if (!LineCheck(pt3.x(), pt3.y(), pt2.x(), pt2.y())) return false;
    if (!LineCheck(pt0.x(), pt0.y(), pt3.x(), pt3.y())) return false;
...
void HybridAStar::SetVehicleShape(double length, double width, double rear_axle_dist) {
    ...
    const auto N_len = static_cast<unsigned int>(length / step);
    const auto N_wid = static_cast<unsigned int>(width / step);
